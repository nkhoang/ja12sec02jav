...
public class VocaAction {
    ...
    public void triggerRemoveWordItemDuplicates(HttpServletResponse response,
                                                @RequestParam int startingIndex,
                                                @RequestParam int size) {
        Queue queue = QueueFactory.getDefaultQueue();
        queue.add(
                TaskOptions.Builder.withUrl("/vocabulary/removeWordItemDuplicates.html")
    ...
    public void removeWordItemDuplicates(@RequestParam int index, @RequestParam int size,
                                         HttpServletRequest request, HttpServletResponse response) {
        List<Long> removeList = new ArrayList<Long>();
        List<WordItem> wordItems = wordItemDao.getAllInRangeWithOrder(index, size, "word");
        String messageBody;
        if (CollectionUtils.isNotEmpty(wordItems)) {
            for (int i = 0; i < wordItems.size(); i++) {
                for (int j = i + 1; j < wordItems.size(); j++) {
                    if (StringUtils.equals(wordItems.get(i).getWord(), wordItems.get(j).getWord())) {
                        removeList.add(wordItems.get(j).getId());
                    } else {
                        i = j;
                        break;
                    }
            ...
            for (Long id : removeList) {
                wordItemDao.delete(id);
            }
            // create mail content.
            Map<String, Object> mailData = new HashMap<String, Object>();
            mailData.put("idList", removeList);
            messageBody = MailUtils.buildMail(request.getSession().getServletContext(), "remove_duplicate.ftl", mailData);
        } else {
            messageBody = "Hi <b>boss</b>, <br /> There is nothing to delete because no record is found.";
        }
        MailUtils.sendMail(messageBody, senderEmail, "Remove Duplicates report", "nkhoang.it@gmail.com");
        try {
            response.getWriter().write("success");
        } catch (Exception e) {
            LOG.error("Error:", e);
    ...
    public void triggerLoadWordItemsFromFile(
            HttpServletResponse response, @RequestParam int startingIndex, @RequestParam int size, @RequestParam int maxIndex) {
        Queue queue = QueueFactory.getDefaultQueue();
