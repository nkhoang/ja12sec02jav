...
public class CurrencyServiceImpl implements CurrencyService {
    ...
    private String getVCBDateFormat(Date date) {
    ...
    public List<Currency> getCurrencyDateFromVCB(Date date, String viewState) {
        ...
        NameValuePair[] parameters = {
                ...
                new NameValuePair("ctl00$Content$BranchList", "68"),
                new NameValuePair("ctl00$Content$DateText", targetDate),
                new NameValuePair("ctl00$Content$ViewButton", "Xem")
        };

        post.setRequestBody(parameters);

        post.setRequestHeader("Host", "www.vietcombank.com.vn");
        post.setRequestHeader("User-Agent", "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_4; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Chrome/8.0.552.231 Safari/534.10");
    ...
    private List<Currency> parseHttpResponse(InputStream is, Date date) {
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(priceTbls)) {
                ...
                List<Element> priceTRs = priceTbl.getAllElements("tr");
