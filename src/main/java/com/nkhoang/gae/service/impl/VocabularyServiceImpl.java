...
public class VocabularyServiceImpl implements VocabularyService {
  ...
  public List<Word> lookupWords(String spreadsheetName, String worksheetName, int row, int col, int size) {
    ...
    if (wordList.size() > 0) {
      ...
      do {
        FutureTask<VocabularyTaskResult> task = new FutureTask<VocabularyTaskResult>(new VocabularyTaskCallable(wordList.get(index)));
        // add task to queue
        taskList.add(task);
  ...
  public void lookupENLongman(Word w) throws IOException {
    ...
    while (source != null) {
      ...
      source = checkWordExistence(
          LONGMAN_DICTIONARY_URL, w.getDescription() + "_" + ++i, LONGMAN_DIC_CONTENT_CLASS, DICTIONARY_TYPE.CLASS);
    }
  }
  ...
  public void lookupPron(Word w) throws IOException {
    ...
    try {
      Source source = checkWordExistence(
          CAMBRIDGE_DICT_URL, w.getDescription().toLowerCase(), CAMBRIDGE_DICT_CONTENT_CLASS, DICTIONARY_TYPE.CLASS);
      int i = 1;
      if (source == null) {
        // check it again
        source = checkWordExistence(
            ...
            if (StringUtils.equals(meaningLi.getName(), "li")) {
              ...
              for (Element content : liContent) {
                if (StringUtils.equals(content.getName(), "b")) {
                  ...
                  String example = content.getChildElements().get(0).getChildElements().get(0).getContent().toString();
