...
public class LuceneUtils {
    ...
    private static final String INDEX_DIR = "lucene/src/test/resources/com/nkhoang/search/index";
    ...
    public static Query buildPharseQuery(String searchString) throws Exception {
    ...
    public static void writeWordToIndex(Word word) throws IOException {
        ...
        if (CollectionUtils.isNotEmpty(word.getMeanings())) {
            for (Meaning m : word.getMeanings()) {
                document.add(new Field(LuceneSearchFields.WORD_CONTENT, m.getContent(), Field.Store.NO, Field.Index.ANALYZED));
            }
