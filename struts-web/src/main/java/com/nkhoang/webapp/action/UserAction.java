...
import com.nkhoang.webapp.helper.ExtendedPaginatedList;
import com.nkhoang.webapp.helper.PaginateListFactory;
...
public class UserAction extends BaseAction implements Preparable {
    ...
    public PaginatedList extendList;

    public PaginatedList getExtendList() {
        return extendList;
    }
    /**
    ...
    public void setPaginateListFactory(PaginateListFactory p) {
        this.paginateListFactory = p;
    }

    public PaginateListFactory getPaginateListFactory() {
        return this.paginateListFactory;
    }
    ...
    public String list() {
        ...
        paginatedList.setPageSize(2);
        int firstIndex =  paginatedList.getFirstRecordIndex();

        log.debug("First index = " + firstIndex);
        List<User> refinedList = new ArrayList<User>(0);
        for (int i = firstIndex; i< firstIndex + 2 && i < users.size(); i++) {
            refinedList.add(users.get(i));
        }

        paginatedList.setList(refinedList);
        paginatedList.setTotalNumberOfRows(users.size());
        flag = "true";

        extendList = paginatedList;
        return SUCCESS;
    }
