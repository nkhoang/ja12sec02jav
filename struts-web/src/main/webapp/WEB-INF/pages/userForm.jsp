...
<select size="1" >
    ...
    <c:forEach var="listItem" items="${aList}" >
        <s:iterator value="aSet" status="index" >
            <c:set var="setItem" >
                <s:property />
            </c:set >
            <c:if test="${setItem eq listItem}">
                <option value="${listItem}" selected="selected">${listItem}</option>
            </c:if>
            <s:if test="#index.last && (#attr['listItem'] != #attr['setItem']) ">
